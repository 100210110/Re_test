...
execute unless score start_countdown time matches 1.. if score game_start mode matches 0 run scoreboard players set start_countdown time 11
execute unless score start_countdown time matches 11 if score game_start mode matches 0 run function re_test:before_game/stop_start_countdown
...
execute if score start_countdown time matches 11 run tellraw @a[team=!setout] [{"text":""},{"text":">>> ","bold":true,"color":"yellow"},{"text":"游戏准备开始 ","bold":true,"color":"aqua"},{"text":">点击返回大厅并准备<","bold":true,"underlined":true,"color":"yellow","clickEvent":{"action":"run_command","value":"/tp @s 203 307 85"},"hoverEvent":{"action":"show_text","contents":[{"text":"点击返回大厅准备","color":"yellow"}]}}]
