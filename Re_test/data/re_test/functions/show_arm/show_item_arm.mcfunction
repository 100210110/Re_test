execute if score show_item_mark mode matches 0 as @e[type=minecraft:armor_stand,tag=sum_mix_mark] run data modify entity @s CustomNameVisible set value 1
execute if score show_item_mark mode matches 1 as @e[type=minecraft:armor_stand,tag=sum_mix_mark] run data modify entity @s CustomNameVisible set value 0
